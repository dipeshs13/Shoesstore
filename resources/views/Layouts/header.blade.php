    ...
    <nav
        ...
        <div class="flex text-2xl space-x-4 mr-10">
            @if(auth()->check())  <!-- Check if user is authenticated -->
                <span class="px-10">{{ auth()->user()->name }}</span> <!-- Display the user's name -->
                <a href="{{ url('/logout') }}" class="hover:bg-black hover:px-10 hover:text-white hover:h-10 transition-colors duration-300 rounded-md">Logout</a>
            @else
                <a href="{{ url('/login') }}" class="hover:bg-black hover:px-10 hover:text-white hover:h-10 transition-colors duration-300 rounded-md">Login</a>
                <a href="{{ url('/register') }}" class="hover:bg-black hover:px-10 hover:text-white hover:h-10 transition-colors duration-300 rounded-md">Register</a>
            @endif
        </div>
    </nav>
